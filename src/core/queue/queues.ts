...
export function getDiscoverQueue(): Queue<StageJobData> {
  if (!discoverQueue) {
    discoverQueue = new Queue<StageJobData>(QUEUE_NAMES.discover, { connection: getRedis() });
  }
...
export function getVerifyQueue(): Queue<StageJobData> {
  if (!verifyQueue) {
    verifyQueue = new Queue<StageJobData>(QUEUE_NAMES.verify, { connection: getRedis() });
  }
  return verifyQueue;
}
